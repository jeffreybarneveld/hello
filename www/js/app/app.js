...
//////////////////////////////////////////////////////////
	   ...
	   ritrecord = new joRecord({ lat          :      "",
				      lon          :      "",
		                      vertrekadres :      "vertrekadres",             //weergave in ritverzamelscherm
		...
		vertrekadresselect = new joCard([new joHTML("<br/>"),
		   ...
		   new joGroup([
			 ...
			 getbutton = new joButton("lees GPS-positie uit").selectEvent.subscribe(function()
				{ //vertaal positie naar een adres
				  var response = AjaxCall("http://tcrcentrale.netshaped.net/10/search/address/"+ritrecord.getProperty("lat")+"/"+ritrecord.getProperty("lon"))
				  //alert("http://tcrcentrale.netshaped.net/10/search/address/"+ritrecord.getProperty("lat")+"/"+ritrecord.getProperty("lon"));
				  //alert(response);
	                          var jsObject = JSON.parse(response);
				  //alert(jsObject);
				  if (jsObject.status==1)
				   { //gevonden!
				     vertrekrecord.setAutoSave(false)
				     vertrekrecord.setProperty("straat"    ,jsObject.straat);
				     vertrekrecord.setProperty("huisnummer",jsObject.huisnummer);
				     vertrekrecord.setProperty("postcode"  ,jsObject.postcode);
				     vertrekrecord.setProperty("plaats"    ,jsObject.plaats);
				     vertrekrecord.setAutoSave(true)
				   }
				}),
		   ...
		   new joGroup([
			 ...
			 getabutton = new joButton("lees GPS-positie uit").selectEvent.subscribe(function()
				{ //vertaal positie naar een adres
				  ...
				  if (jsObject.status==1)
				   { //gevonden!
				     aankomstrecord.setAutoSave(false)
				     aankomstrecord.setProperty("straat"    ,jsObject.straat);
				     aankomstrecord.setProperty("huisnummer",jsObject.huisnummer);
				     aankomstrecord.setProperty("postcode"  ,jsObject.postcode);
				     aankomstrecord.setProperty("plaats"    ,jsObject.plaats);
				     aankomstrecord.setAutoSave(true)
				   }
				}),
...
setTimeout( "navigator.geolocation.getCurrentPosition(onSuccess, onError)",2000); //pas na 2 sec positie bepalen en opslaan. JoApp moet gestart zijn
...
function onSuccess(position)
 {
   ...
   dm.setProperty('lat',lat)
   dm.setProperty('lon',lon)
   
   //again after 10 sec
   setTimeout( "navigator.geolocation.getCurrentPosition(onSuccess, onError)",10000);
/**        
